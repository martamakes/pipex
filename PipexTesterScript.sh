...
# Test 10: Command not found in PATH
OLD_PATH=$PATH
export PATH=""
...
# Test 11: Input file is a directory
mkdir testdir
...
# Test 12: Output file is a directory
mkdir outdir
...
if command -v valgrind &> /dev/null; then
    ...
    valgrind --leak-check=full --show-leak-kinds=all --error-exitcode=1 ./pipex infile "cat" "wc -l" outfile > output/valgrind_output 2>&1
    if [ $? -eq 0 ]; then
...
echo "Cleanup script created. Run './cleanup.sh' to remove test files and clean the project."
